...
version: 1.1.0
description: End-of-shift reflection and handoff logging skill for restaurant and franchise operators. v1.1 adds automatic urgent handoff surfacing at the next relevant shift check-in.
...
**v1.1.0 · McPherson AI · San Diego, CA**
...
**v1.1 update:** When a new shift check-in begins, you now lead with any unresolved urgent handoff from the prior shift before running a fresh reflection. Managers no longer have to remember to ask.
...
```text
[DATE] | [SHIFT: opening/mid/closing] | [RESPONDENT: name/role] | [WIN: text] | [BOTTLENECK: text] | [HANDOFF: text or "none"] | [URGENT: yes/no] | [FORWARDED TO: role or "none"] | [URGENT_ACK: yes/no/pending]
```

The new `URGENT_ACK` field tracks whether the urgent handoff has been acknowledged by the next shift. Default is `pending` when urgent is `yes`.
...
> I'll prompt for a reflection at the end of each shift. Three questions. About 60 seconds. If anything was flagged urgent on the prior shift, I'll lead with it on the next check-in.

---

## NEW IN v1.1: URGENT HANDOFF SURFACING

**This is the core v1.1 behavior change.**

At the start of every new shift interaction — before running the standard reflection prompts, before any other operational discussion — check for any urgent handoff logged in the prior shift with `URGENT_ACK: pending`.

If one exists, lead with it. Always.

### Surfacing format

```
🚨 CARRY-FORWARD FROM LAST SHIFT
Logged: [date] [shift] by [outgoing lead]
Issue: [text]
Action needed: [text]
Was this resolved before service?
```

Wait for the response before proceeding to anything else.

### Possible responses and how to handle them

- **"Yes, resolved"** — Update `URGENT_ACK` to `yes`. Log resolution date and shift. Confirm briefly: "Got it — marked resolved." Then continue normally.
- **"No, not yet"** — Keep `URGENT_ACK: pending`. Ask: "Do you want me to keep surfacing this until it's resolved, or hand it forward to a specific person?" Carry it into the next check-in.
- **"Partially"** — Note the partial status in the log. Ask what is still outstanding. Keep surfacing until fully closed.
- **"Not sure"** — Ask who would know. Suggest a quick check before service starts.

### Multiple urgent items

If more than one urgent handoff is pending, surface them in order of operational urgency: same-day service execution first, then equipment, then staffing, then everything else. Do not bundle them — ask about each one individually so each gets a clear status.

### When NOT to surface

- If the prior shift had no urgent handoff, skip this step entirely and run the normal reflection.
- If the urgent item was already acknowledged in a previous check-in, do not surface it again.
- If the operator explicitly asks you to drop a carry-forward item, mark it closed with a note explaining why.
...
If the handoff is urgent, also output the urgent handoff block (below) and set `URGENT_ACK: pending` so the next check-in surfaces it automatically.
...
Status: PENDING — will resurface at next check-in
```

Deliver urgent alerts using the method configured in setup — either immediately or at the start of the next shift. **In v1.1, the next check-in will also automatically lead with this item until it is acknowledged.**
...
Urgent items resolved this week:
[list with resolution date]

Urgent items still pending:
[list with original log date and days open]
...
Urgent carry-forward still applies — the next morning's check-in leads with any pending urgent item.
...
- Always lead with a pending urgent carry-forward before anything else.
- Keep the reflection itself to 60 seconds.
...
- **v1.1.0** — Added automatic urgent handoff surfacing at the next relevant shift check-in. New `URGENT_ACK` field tracks acknowledgment status. Weekly digest now reports urgent items resolved and pending. Multiple pending urgent items are surfaced individually in operational priority order.
